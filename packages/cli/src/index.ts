...
new Command()
    ...
    .addCommand(
        new Command('run')
            ...
            .addOption(new Option('--output, -o <string>', 'Path the the bundle output directory.').default(DEFAULT_SCAN_OUTPUT).env('NOODLE_OUTPUT'))
    ...
    .addCommand(
        new Command('serve')
            ...
            .addOption(new Option('-p, --port <number>', 'Port to listen on').default(0).env('NOODLE_PORT'))
            ...
            .addOption(new Option('--open', 'Whether to open in the browser the UI when ready.'))
            .action(serve)
    )
    .parse();
